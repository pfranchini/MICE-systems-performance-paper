...
\begin{figure}[!ht]
  ...
  \caption{Design of the TOF detector mechanical structure~\cite{Rayner:2011zz} showing the horizontal and vertical slabs (left) and details of each slab~\cite{NOTE241} showing one scintillator with on each side the light-guides and the PMTs (right).}
...
respectively.  Each of the planes in TOF0 had 10, 4\,cm wide,
...
measurement is essential for time-walk corrections. Each electronic
...
MICE beam are muons, pions, and positrons. The time resolution needed
to be sufficiently good to effectively discriminate between these
types. At 24\,MeV/c, the time-of-flight difference between muon and
pion was about 1.\,ns between TOF0 and TOF1 stations. With 20\,ps
resolution, one reached near 100\% discrimination efficiency.
...
of each station.
The time associated to each TOF is determined from the average of the times when the two slabs (horizontal and vertical) are crossed by a particle.
The resolution of the average is a half of the spread of their difference. Therefore, looking at slab \DT{} allows the time-of-flight
...
approximately~0.11\,ns is about 30\%~larger than the calculated spread from a
naive addition of slab \DT{} of the individual TOF stations.
This difference is attributed to contributions to the resolution of
